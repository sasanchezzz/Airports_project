...
PASSENGER_ID_PATTERN = re.compile(r"^\d{4}\s\d{6}$")
PASSENGER_ID_DIGITS_PATTERN = re.compile(r"^\d{10}$")
PASSENGER_NAME_PATTERN = re.compile(
    r"^[A-Za-z]+ [A-Za-z]+(-[A-Za-z]+)?$"
)
EMAIL_PATTERN = re.compile(r"^[^@]+@[^@]+\.[^@]+$")
PHONE_PATTERN = re.compile(r"^\+\d{10,15}$")
BOOK_REF_PATTERN = re.compile(r"^[A-Z0-9]+$")
...
class TicketsBase(BaseModel):
    ...
    def check_passenger_id(
        ...
        try:
            ...
            if not PASSENGER_ID_PATTERN.match(clean_value):
                raise ValueError(
            ...
            if isinstance(value, (int, float)):
                ...
                if PASSENGER_ID_DIGITS_PATTERN.match(digits_only):
                    return cls.check_passenger_id(
    ...
    def check_passenger_name(cls, value: str) -> str:
        ...
        if not PASSENGER_NAME_PATTERN.match(value):
            raise ValueError(
    ...
    def check_contact_data(
        ...
        Проверяет что словарь содержит только ключи 'email' и 'phone', и что их значения соответствуют формату

        Поле 'email' должно быть в формате 'user@domain.com'
        Поле 'phone' должно быть в формате '+1234567890'
        ...
        if "email" in value and not EMAIL_PATTERN.match(
            value["email"]
        ):
            raise ValueError(
                f"Invalid format for email: {value['email']}"
            )

        if "phone" in value and not PHONE_PATTERN.match(
            value["phone"]
        ):
            raise ValueError(
                "Phone must start with '+' and contain 10-15 numbers"
            )
...
class TicketsResponse(BaseModel):
    ...
    def check_book_ref(cls, value: str) -> str:
        ...
        if not BOOK_REF_PATTERN.match(value):
            raise ValueError(
