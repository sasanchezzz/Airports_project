...
class TicketsCreate(BaseModel):
    ...
    @field_validator("passanger_id", mode="wrap")
    @classmethod
    def check_passanger_id(
        ...
        try:
            ...
            if not isinstance(result, str):
                raise ValueError("Passanger_id must be string")
    ...
    @field_validator("passanger_name", mode="after")
    @classmethod
    def check_passanger_name(cls, value: str) -> str:
        """
        Валидатор для поля passanger_name
        ...
        if not value:
            raise ValueError("Passanger_name can't be empty!")

        if not isinstance(value, str):
            raise ValueError("Passanger_name must be a string!")

        value = value.strip()

        if not re.match(r"^[A-Za-z]+ [A-Za-z]+(-[A-Za-z]+)?$", value):
            raise ValueError(
                "Passanger_name must be in format: 'NAME SURNAME' or 'NAME SURNAME1-SURNAME2'"
            )
    ...
    def check_contact_data(
        ...
        if not isinstance(value, dict):
            raise ValueError("Contact_data msut be dictionary!")
